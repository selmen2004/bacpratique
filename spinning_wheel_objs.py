...
#             self.useditems.append(result_item)
#             self.names.append(tk.user_controls.name_eleve.get("1.0"))
#             self.items.delete(result_item)
            #self.draw()
            return result_item
...
class Exam:
    def get_num_eleves():
        ...
        if datetime.today().weekday() == 1 or datetime.today().weekday() == 6 : #le cas de section SI
            return 15
        else: #les autres sections
            return 13
    def next_exam():
        from datetime import datetime
        if datetime.today().weekday() == 1 or datetime.today().weekday() == 6 : #le cas de section SI
            import datetime
            examtime = datetime.datetime.now().replace(hour=8,minute=30 , second=0)
            return(examtime,90)
            ...
            examtime = datetime.datetime.now().replace(hour=8,minute=0 , second=0)
            if timenow < examtime :
                return(examtime,60)
            examtime = datetime.datetime.now().replace(hour=9,minute=30 , second=0)
            if timenow < examtime:
                return(examtime,60)
            examtime = datetime.datetime.now().replace(hour=11,minute=0 , second=0)
            if timenow < examtime:
                return(examtime,60)
            examtime = datetime.datetime.now().replace(hour=13,minute=0 , second=0)
            if timenow < examtime:
                return(examtime,60)
            examtime = datetime.datetime.now().replace(hour=14,minute=30 , second=0)
            if timenow < examtime:
                return(examtime,60)
    ...
    def curr_exam():
        ...
        if datetime.today().weekday() == 1 or datetime.today().weekday() == 6 : #le cas de section SI
            ...
            if examtime  + datetime.timedelta(hours=1) >= timenow >= examtime :
                return(examtime,60)
            examtime = datetime.datetime.now().replace(hour=17,minute=41 , second=0)
            if examtime  + datetime.timedelta(hours=1) >= timenow >= examtime :
                return(examtime,60)
            print("No remaining Exams")
